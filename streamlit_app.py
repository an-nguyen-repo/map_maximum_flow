...
class TrafficFlowUI:
    ...
    def load_data(self):
        ...
        try:
            ...
            self.index_node_lookup = {
                v: int(k) for k, v in self.node_index_lookup.items()
            }
    ...
    def draw_edge(self, m, node_fr_id, node_to_id, color=ACTIVE_COLOR):
        coordinates = [
            [
                self.G.nodes[node_fr_id]["lat"],
                self.G.nodes[node_fr_id]["lon"],
            ],  # lat, long
    ...
    try:
        ...
        with col1:
            ...
            algorithm = st.selectbox(
                "Algorithm", options=["Edmonds-Karp", "Dinic", "Ford-Fulkerson"]
            )
        ...
        st.error(
            "Please ensure all required data files are present and properly formatted."
        )
